...
class Notebook(UserDict):
    ...
    def put_note_to_db(note: Note):
        crud_note.create_note(title=note.name, description=note.text_note, tags_name_list=note.tags)
    ...
    def edit_note_tag_add(name: str, tag: str):
        crud_note.create_tag(note_title=name, tag_name=tag)
        return f'Tag <{tag}> in note "{name}" "added'

    @staticmethod
    def edit_note_tag_del(name: str, tag: str):
        if crud_note.remove_tag(note_title=name, tag_name=tag):
            return f'Tag <{tag}> in note "{name}" deleted'
        else:
            return f'Tag <{tag}> is not exist'
    ...
    def find_note_by_tag(tag_name: str):
        if not crud_note.exist_tag(name=tag_name) and len(tag_name) > 0:
            return f'Tag <{tag_name}> not exist'
        find_result = []
    ...
    @staticmethod
    def show_note_by_tag(tag: str = None):
        result = ''
        tags_db = crud_note.tags_all()

        for tag_db in tags_db:
            notes_db = crud_note.find_note_by_tag(tag_name=tag_db.name)
            notes = []
            for note_db in notes_db:
                notes.append(f'"{note_db.title}"')
            result += f'<{tag_db.name}>: ' + ', '.join(notes) + '\n'

        notes_db = crud_note.find_note_by_tag(tag_name="")
        notes = []
        for note_db in notes_db:
            if not note_db.tags:
                notes.append(f'"{note_db.title}"')
